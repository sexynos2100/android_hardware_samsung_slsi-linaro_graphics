          ...
          m_pStreamBase(NULL), m_fThumbBufferType(0)
{
    ...
    ALOGI("ExynosJpegEncoderForCamera Created: %p, ION %d", this, m_fdIONClient);
}
...
ExynosJpegEncoderForCamera::~ExynosJpegEncoderForCamera()
{
    ...
    ALOGI("ExynosJpegEncoderForCamera Destroyed: %p, ION %d, ThumIMG %d",
                                             ...
                                             extra_appinfo_t *extra)
{
    ...
    ALOGI("Image compression starts from offset %zu (APPx size %zu, HWFC? %d, NBTB? %d)",
                                       ...
                                       extra_appinfo_t *appInfo)
{
    ...
    ALOGI("Given stream buffer size: %d bytes", *size);
    ...
    *size = static_cast<int>(FinishCompression(mainlen, thumblen));
    ...
    ALOGI("....compression delay(usec.): HW %u, Total %lu)",
          GetHWDelay(), stopwatch.GetElapsed());

    if (property_get_bool("vendor.jpeg.debug", false))
        DumpInfo();
...
ssize_t ExynosJpegEncoderForCamera::FinishCompression(size_t mainlen, size_t thumblen)
{
     ...
     */
    ALOGI("Completed image compression (%zd(thumb %zu) bytes, HWFC? %d, BTB? %d)",
...
bool ExynosJpegEncoderForCamera::GenerateThumbnailImage()
{
    ...
    ALOGI("Generating thumbnail image: %dx%d -> %dx%d",
...
size_t ExynosJpegEncoderForCamera::GetThumbnailImage(char *buffer, size_t buflen)
{
    ...
    ALOGI("Copied thumbnail image to %p (%zu bytes)", buffer, thumbbufsize);
...
void ExynosJpegEncoderForCamera::DumpInfo() {
    bool thumbenc = m_pAppWriter->GetThumbStreamBase() != NULL;
    bool btb = !TestState(STATE_NO_BTBCOMP) && IsBTBCompressionSupported() && !IsThumbGenerationNeeded();
    int width, height;

    getSize(&width, &height);

    ALOGI("btb(%d) hwfc(%d) main(%dx%d) thumb(%dx%d)",
            btb, TestState(STATE_HWFC_ENABLED), width, height, m_nThumbWidth, m_nThumbHeight);

    GetCompressor().DumpInfo(false, btb);

    if (thumbenc && IsThumbGenerationNeeded())
        m_phwjpeg4thumb->DumpInfo(true, false);
